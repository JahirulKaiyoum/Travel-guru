const fakePlaces = [
    {
        name: "Cox'sBazar",
        description: "Cox's Bazar (Bengali: কক্সবাজার) is a town, a fishing port and district headquarters in Bangladesh. It is known for its wide sandy beach which is the world's longest natural sandy sea beach. It is an unbroken 125 km sandy sea beach with a gentle slope. It is located 150 km south of Chittagong. Cox’s Bazar is also known by the name Panowa, the literal translation of which means yellow flower",
        id: 1,
        img: 'https://i.ibb.co/k4M9tV8/Sajek.png',
        Latitude: 21.427229,
        Longitude:92.005806
        
    },
    {
        name: "Sreemangal",    
        description: 'It is said the name Sreemangal (or Srimangal) is named after Sri Das and Mangal Das; two brothers who settled on the banks of the Hail Haor.[2] A copper plate of Raja Marundanath from the 11th century was found in Kalapur. During an excavation at Lamua, an ancient statue of Ananta Narayan was dug out. In 1454, the Nirmai Shiva Bari was built and still stands today. Srimangal thana was established in 1912.', 
        id: 2,
        img: 'https://i.ibb.co/0CgNcZR/Sreemongol.png',
        Latitude: 24.310577,
        Longitude:91.725136

    },
    {
        name: "Sundarban",    
        description: 'The Sundarbans is a mangrove area in the delta formed by the confluence of the Ganges, Brahmaputra and Meghna Rivers in the Bay of Bengal. It spans from the Hooghly River in India state of West Bengal to the Baleswar River in Bangladesh. It comprises closed and open mangrove forests, agriculturally used land, mudflats and barren land, and is intersected by multiple tidal streams and channels', 
        id: 3,
        img: 'https://i.ibb.co/k4tLJCC/sundorbon.png',
        Latitude: 21.9497,
        Longitude:89.1833
    }

    
]

export default fakePlaces;